...
class GeneratorBasedBuilder(FileReaderBuilder):
  ...
  def _generate_examples(
    ...
    * A `beam.PTransform`: For big datasets and distributed generation. See our
    Apache Beam
    ...
    * A `beam.PCollection`: This should only be used if you need to share some
    ...
    def _split_generators(self, dl_manager, pipeline):
      ...
      pipeline |= 'SharedPreprocessing' >> beam.Map(...)
      ...
      ...
      return {
          'train': pipeline | 'train' >> generate_examples(train_path)
          'test': pipeline | 'test' >> generate_examples(test_path)
      }

    def _generate_examples(self, pipeline, path):
      return (
          pipeline
          | beam.Create(path.iterdir())
          | beam.Map(lambda filepath: filepath.name, {'image': ..., ...})
